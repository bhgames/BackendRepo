					...
					while(i<towns().size()) {
						...
						if(Math.abs(idT.getX()-x)<=idInfluenceDistance&&Math.abs(idT.getY()-y)<=idInfluenceDistance&&!idT.isResourceOutcropping()) { // FIND ANY ID TOWN THAT CAN SEND THE ATTACK.
							...
							b = idT.addBuilding("Command Center",4,1,0);
							 ...
							 System.out.println(
							 getPs().b.attack(idT.townID,t.getX(),t.getY(),auAmts,"siege",null,"GARBLEDTRANSMISSION",true) + ":" + getPs().b.getError());
					...
					if(rand>0&&rand<.09) {
						...
						if(idT.getAu().get(0).getSize()>0) {
							...
							b = idT.addBuilding("Command Center",4,1,0);
							 ...
							 int auAmts[] = {idT.getAu().get(0).getSize(),0,0,0,0,0};
							 getPs().b.attack(idT.townID,t.getX(),t.getY(),auAmts,"attack",null,"GARBLEDTRANSMISSION",true);
						...
						idT.setSize(0,(int) Math.round(totalNum*.3));
							...
							b = idT.addBuilding("Command Center",4,1,0);
							 ...
							 int auAmts[] = {idT.getAu().get(0).getSize(),0,0,0,0,0};
							 getPs().b.attack(idT.townID,t.getX(),t.getY(),auAmts,"siege",null,"GARBLEDTRANSMISSION",true);
